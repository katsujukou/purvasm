...
let test_log_force_performs () =
  check_eperf
    ...
    (A.Let
       ...
       , log_app "x"
       , A.Let
           ("r", A.CApp (A.AVar "p", [ A.ALit (LInt 0) ]), A.Ret (A.CAtom (A.AVar "r")))
       ))
...
let test_newarray_setarray_perform () =
  check_eperf
    "new[]"
    true
    (A.Ret (A.CPrim (NewArray, [ A.ALit (LInt 3); A.ALit (LInt 0) ])));
  check_eperf
    "set[]"
    true
    (A.Let
...
let test_pure_function () =
  ...
  check_eperf
    ...
    (A.Let
       ...
       , f
       , A.Let
           ("r", A.CApp (A.AVar "f", [ A.ALit (LInt 3) ]), A.Ret (A.CAtom (A.AVar "r")))
       ))
...
let test_multiarg_effectful_leaf () =
  ...
  Alcotest.(check bool)
    "runEffectFn2 f a b builds (pure)"
    false
    (a.E.eperf (A.Ret (rt2 [ i 0; i 1; i 2 ])));
  (* RT2 f a  — partial application: pure build *)
  Alcotest.(check bool)
    "runEffectFn2 f a (PAP) is pure"
    false
    (a.E.eperf (A.Ret (rt2 [ i 0; i 1 ])));
  (* (RT2 f a b) ()  — forcing the built thunk performs *)
  Alcotest.(check bool)
    "forcing runEffectFn2's result performs"
    true
    (a.E.eperf (A.Let ("t", rt2 [ i 0; i 1; i 2 ], A.Ret (A.CApp (A.AVar "t", [ i 9 ])))))
...
let test_deep_returned_closure_chain () =
  ...
  check_eperf
    "f 0 1 2 (flattened) performs"
    true
    (A.Let
       ( "f"
       , f_lam
       , A.Let ("r", A.CApp (A.AVar "f", [ i 0; i 1; i 2 ]), A.Ret (A.CAtom (A.AVar "r")))
       ));
  (* staged: r1 = f 0; r2 = r1 1; r3 = r2 2 *)
  check_eperf
    "f 0; · 1; · 2 (staged) performs"
    true
    (A.Let
       ( "f"
       , f_lam
...
let test_point_free_arity () =
  ...
  let i n = A.ALit (LInt n) in
  let with_inc tail =
    A.Let ("add", add, A.Let ("inc", A.CApp (A.AVar "add", [ i 1 ]), tail))
  in
  ...
  check_eperf
    "inc 5 (exact saturation) is pure"
    false
  ...
  Alcotest.(check bool)
    "point-free logIt = log \"x\" is an effect thunk"
    true
    (E.M.find "logIt" lsums).E.vsat;
  check_eperf
    "forcing point-free logIt performs"
    true
    (with_log
       (A.Let ("u", A.CApp (A.AVar "logIt", [ i 0 ]), A.Ret (A.CAtom (A.AVar "u")))))
...
let is_ret0 = function
  | A.Ret (A.CAtom (A.ALit (LInt 0))) -> true
  | _ -> false

let is_let name = function
  | A.Let (n, _, _) -> String.equal n name
  | _ -> false
...
let test_dbe_drops_dead_pure () =
  Alcotest.(check bool)
    "drop dead pure let"
    true
    (is_ret0 (dbe (A.Let ("x", add12, ret0))))
...
let test_dbe_drops_effect_build () =
  Alcotest.(check bool)
    "drop dead Effect build (I1)"
    true
    (is_ret0 (dbe (A.Let ("p", log_app "x", ret0))))
...
let suite =
  ...
  ; Alcotest.test_case
      "unknown application conservative"
      `Quick
      test_unknown_application_conservative
  ; Alcotest.test_case "recursive effect fixpoint" `Quick test_recursive_effect_fixpoint
  ; Alcotest.test_case
      "multi-arg effectful leaf (arity-aware)"
      `Quick
      test_multiarg_effectful_leaf
  ; Alcotest.test_case
      "deep returned-closure chain (>=2 levels)"
      `Quick
      test_deep_returned_closure_chain
