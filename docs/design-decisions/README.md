...
| [0086](0086-optimizer-codegen-seam-two-phase-module-pipeline.md) | The optimiser/codegen seam over a backend-neutral ANF module (+ 2026-07-10 **Addendum**: remove the always-on shared `DictElim` bridge — the neutral driver runs no `DictElim`; `--opt` runs it in the optimiser fixpoint; native `--no-opt` keeps a **backend-private** transitional bridge in `llvmBackend`; the VM runs none, so its `--no-opt` stays normalise-only) | Accepted |
| [0087](0087-backend-neutral-build-driver-compileraction.md) | A backend-neutral build driver over an injected `CompilerAction` — lifts orchestration (load-closure + topsort + the ADR-0086 fold) out of the CLI/backend into one `compiler`-library `build`, over a **pure `Backend o`** (`lowerModule` + whole-program `lowerEntry`) and an **effectful `CompilerAction o m`** (host `loadModule`/`emitFile`/`emitEntry` + per-phase inspection hooks); `loadModule` returns an explicit `LoadResult` (`Loaded`/`Missing`/`Failed`) and `build` returns `m (Either BuildError (BuildProducts o))` so `m` stays `Monad` (no `MonadError`); link stays in the CLI per target (VM spine-fold / native `clang`/`lld`), a false-unification `link` capability rejected. **§3.1**: `--emit-ir <module>` is a per-round ANF trace (not a stop), `--opt-max-iter` bounds the fixpoint | Accepted |
| [0088](0088-vm-backend-lowers-like-native-release-boot-byte-identity.md) | The VM backend lowers the optimiser seam like native, released from boot byte-identity — retire the **`.pmo`/`app.pvm == boot` gates only** (`.pmi` unchanged, still byte-identical to boot for native, ADR-0084); `bytecodeBackend` mirrors `llvmBackend` (consumes the driver's **seam-output** `AnfModule`; `deps` = a decl's free **global** refs from the ANF (reachability edges), `foreignRefs` separate (required foreigns, not graph nodes); **`lowerEntry` owns reachability + the initialiser**, link is assembly-only); `.pmo`/`.pvm` become **decls + init** (REPL-consumable); VM regated **behaviourally** (run vs oracle / VM≡LLVM≡JS), boot kept as a behaviour reference only. Unblocks the VM re-homing (ADR-0087 §5) and makes the VM the optimiser measurement field | Accepted |
