...
| [0082](0082-native-codegen-port-to-level-2.md) | Port the native LLVM backend to Level-2 PureScript (wall 3), gate split by layer: the backend (codegen_llvm ~2465 loc) + required lowering (DictElim) is a faithful `.ll`-byte-identical transcription vs frozen boot; the optimiser (Simplify/Dbe + a new best-in-class **NbE general inliner**) is rebuilt and gated on behavioural `--opt ≡ --no-opt ≡ oracle` equivalence — separated by a `--no-opt` flag on both Level 2 and boot; two parallel tracks joined at the native self-host; consumes ADR-0080 shapes | Proposed |
| [0083](0083-match-compilation-to-anf-middle-end.md) | Move match/decision-tree compilation out of the **three** per-backend matchers (bytecode = ADR-0031 decision tree; `codegen_ml` = record-binder hybrid; LLVM = CPS cascade, no tree) into ONE shared ANF→ANF pass (Maranget), in **both boot and Level-2**, positioned late (after the optimiser, before codegen) so the NbE inliner sees structured `case`; re-baseline bytecode byte-identity (both change together) + 4-way differential; a one-time boot-freeze exception done now while boot is the live cross-check; keeps `codegen_ml` as the last insurance leg (retired once native self-host subsumes it); defers ADR-0082 (native port then just lowers the decision tree) | Accepted |
