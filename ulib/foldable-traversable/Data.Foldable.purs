-- | ulib SHADOW of `foldable-traversable`'s `Data.Foldable` (ADR-0038), targeting 6.0.0.
-- | Byte-for-byte upstream EXCEPT `foldrArray` / `foldlArray`, which are PureScript over the
-- | purvasm-base array primitives (`Purvasm.Array` + prelude-free `Purvasm.Int`) so the folded closure
-- | specializes (ADR-0027) instead of going through the opaque foreign. Interface unchanged.
...
-- ulib shadow: array folds over purvasm-base primitives, so the folded closure specializes on purvasm
-- (vs the opaque foreign). `foldl` ascends, `foldr` descends; both index in place (no build).
foldrArray :: forall a b. (a -> b -> b) -> b -> Array a -> b
foldrArray f z xs = go (PA.length xs) z
  where
  go i acc = if PI.eq i 0 then acc else let j = PI.sub i 1 in go j (f (PA.unsafeIndex xs j) acc)

foldlArray :: forall a b. (b -> a -> b) -> b -> Array a -> b
foldlArray f z xs = go 0 z
  where
  n = PA.length xs
  go i acc = if PI.eq i n then acc else go (PI.add i 1) (f acc (PA.unsafeIndex xs i))
