-- | ulib SHADOW of `unfoldable`'s `Data.Unfoldable1` (ADR-0038), targeting unfoldable 6.0.0.
-- |
-- | The upstream module's interface (exports, class, instances) is unchanged, so this shadows the
-- | registry module at the user's resolved version. The one foreign, `unfoldr1ArrayImpl`, is
-- | reimplemented in PureScript over the `purvasm-base` array primitives: since there is no
-- | byte-level `push`, the JS `result.push` loop becomes a growable array (capacity doubled on
...
-- ulib shadow: the growable-array build (was prelude's opaque `foreign import unfoldr1ArrayImpl`).
-- `unfoldr1` always yields at least one element, so each step pushes `fst (f value)` before testing
-- the `Maybe` continuation.
unfoldr1ArrayImpl
  :: forall a b
  ...
  -> b
  -> Array a
unfoldr1ArrayImpl isNothing' fromJust' fst' snd' f = \b -> go b (PA.unsafeNew 1) 0
  where
  go value buf count =
    let
      tuple = f value
      ...
      next = snd' tuple
      count' = PI.add count 1
    in
      if isNothing' next then trimTo buf'' count'
      else go (fromJust' next) buf'' count'

-- A growable array over the purvasm-base primitives: `grow` returns a buffer with room for one more
-- element (doubling the capacity when full), `trimTo` copies the used prefix into an exact-length
-- array. `blit` threads the destination so each write stays live and ordered by the data dependency.
