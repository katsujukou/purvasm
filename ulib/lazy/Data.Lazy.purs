...
-- |
-- | This is the purvasm `ulib` shadow (ADR-0038, demanded by the CST parser's
-- | `TokenStream`): the upstream foreign trio becomes a pure-PureScript memoising
-- | thunk over `Effect.Ref` — computed at most once, exactly the upstream contract —
-- | and every instance below is upstream's, verbatim (instance contexts are frozen
-- | ABI). The memo cell is per-`defer`; re-forcing returns the saved value.
...
defer f =
  let
    cell = unsafePerformEffect (Ref.new Nothing)
  in
    Lazy \_ -> unsafePerformEffect do
      Ref.read cell >>= case _ of
        Just v -> pure v
        Nothing -> do
          let v = f unit
          Ref.write (Just v) cell
          pure v
