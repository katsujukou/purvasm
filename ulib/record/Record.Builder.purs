-- | ulib shadow of `Record.Builder` (the `record` package, ADR-0038/0065): the upstream module's five
-- | `foreign import`s are **JS-only**, so on the native backend a reference to them is an unresolved
-- | foreign — the self-host closure reaches `Record.Builder` via `argparse-basic` / `record-studio`.
-- | Here they are reimplemented as pure PureScript over the resolved `Record.Unsafe` primops
-- | (`unsafe{Get,Set,Delete}` → `RecordGet`/`RecordSet`/`RecordDelete`), so the module carries **no
-- | foreign** and resolves on every backend (ADR-0094: the structural body moves to the library, folded
-- | through the ordinary candidate channel). This mirrors boot's `Ffi` for the same five keys —
-- | `copyRecord` = identity, `unsafeInsert` = `RecordSet`, `unsafeDelete` = `RecordDelete`, `unsafeModify`
-- | = `RecordSet ∘ RecordGet`, `unsafeRename` = `RecordSet ∘ RecordGet ∘ RecordDelete`.
-- |
-- | Everything below the five reimplemented helpers is **verbatim** upstream `Record.Builder` (v4.0.0):
-- | the public surface — exports, signatures, and the `Semigroupoid`/`Category` instances (their context
-- | and declaration order are a frozen ABI a consumer's CoreFn bakes, [ulib-instance-context-abi]) — is
-- | unchanged, so this is drop-in.
...
import Record.Unsafe (unsafeDelete, unsafeGet, unsafeSet) as Unsafe
import Record.Unsafe.Union (unsafeUnionFn)
import Type.Proxy (Proxy)
import Unsafe.Coerce (unsafeCoerce)

-- The five upstream `foreign import`s, as pure PureScript over `Record.Unsafe` (see the module header).
-- purvasm records are immutable (`record_set` returns a fresh record, ADR-0069), so no `Builder`
-- mutation is observable and `copyRecord` is the identity — boot's `Ffi` resolves it the same way.

copyRecord :: forall r1. Record r1 -> Record r1
copyRecord r1 = r1

unsafeInsert :: forall a r1 r2. String -> a -> Record r1 -> Record r2
unsafeInsert = Unsafe.unsafeSet

unsafeModify :: forall a b r1 r2. String -> (a -> b) -> Record r1 -> Record r2
unsafeModify l f r1 = Unsafe.unsafeSet l (f (Unsafe.unsafeGet l r1)) r1

unsafeDelete :: forall r1 r2. String -> Record r1 -> Record r2
unsafeDelete = Unsafe.unsafeDelete

unsafeRename :: forall r1 r2. String -> String -> Record r1 -> Record r2
unsafeRename l1 l2 r1 = Unsafe.unsafeSet l2 (Unsafe.unsafeGet l1 r1) (Unsafe.unsafeDelete l1 r1)
