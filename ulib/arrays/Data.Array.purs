-- | ULIB SHADOW of `arrays`' `Data.Array` (ADR-0038), targeting arrays 7.3.x.
-- |
-- | Fidelity by copy: this is the registry module verbatim, with only the higher-order
-- | `*Impl` foreigns (`filterImpl`, `partitionImpl`, `zipWithImpl`, `scanlImpl`/`scanrImpl`,
-- | `findIndexImpl`/`findLastIndexImpl`/`findMapImpl`, `anyImpl`/`allImpl`) reimplemented in
-- | PureScript over `Purvasm.Array` primitives. On purvasm those closures specialize (ADR-0027), so
-- | idiomatic `filter`/`zipWith`/… run without the opaque `call_ref`-per-element of a foreign
-- | HOF; the public interface (exports + types) is unchanged. Index arithmetic uses ordinary
-- | `Prelude` `Int` ops — `Data.Array` sits above `Prelude`, and those ops are already
-- | intrinsics on this backend, so no `Purvasm.Int` is needed (unlike the sub-`Prelude` shadows).
-- | Structural foreigns (`range`/`replicate`/`reverse`/`slice`/…) and the `Data.Array.ST`-based
-- | functions are kept as-is — same behaviour as the registry, resolved/eliminated identically.
-- |
...
replicate n x = go 0 (PA.unsafeNew len)
  where
  len = if n < 0 then 0 else n
  go i out = if i == len then out else go (i + 1) (PA.unsafeSet out i x)
...
findMap f xs = go 0
  ...
  go i =
    if i == n then Nothing
    else case f (PA.unsafeIndex xs i) of
      Nothing -> go (i + 1)
      m -> m
...
findIndex f xs = go 0
  ...
  go i =
    if i == n then Nothing
    else if f (PA.unsafeIndex xs i) then Just i
    else go (i + 1)
...
findLastIndex f xs = go (PA.length xs - 1)
  where
  go i =
    if i < 0 then Nothing
    else if f (PA.unsafeIndex xs i) then Just i
...
insertAt idx x xs =
  let
    ...
    go j out =
      if j == n + 1 then out
      else if j < idx then go (j + 1) (PA.unsafeSet out j (PA.unsafeIndex xs j))
      else if j == idx then go (j + 1) (PA.unsafeSet out j x)
      else go (j + 1) (PA.unsafeSet out j (PA.unsafeIndex xs (j - 1)))
  in
    if idx < 0 || idx > n then Nothing else Just (go 0 (PA.unsafeNew (n + 1)))
...
updateAt idx x xs =
  let
    ...
    go j out =
      if j == n then out
      else go (j + 1) (PA.unsafeSet out j (if j == idx then x else PA.unsafeIndex xs j))
  in
    if idx < 0 || idx >= n then Nothing else Just (go 0 (PA.unsafeNew n))
...
concat xss = fill 0 0 (PA.unsafeNew total)
  where
  outer = PA.length xss
  total = sumLen 0 0
  sumLen i acc = if i == outer then acc else sumLen (i + 1) (acc + PA.length (PA.unsafeIndex xss i))
  fill i pos out =
    if i == outer then out
    else
      let
        inner = PA.unsafeIndex xss i
        n = PA.length inner
      in
        fill (i + 1) (pos + n) (copyInto inner 0 n pos out)
  copyInto src j n pos out =
    if j == n then out
    else copyInto src (j + 1) n pos (PA.unsafeSet out (pos + j) (PA.unsafeIndex src j))
...
-- Count matches first, then fill an exact-size buffer in one pass. The result size of a
-- filter is not known up front, but we must allocate `Purvasm.Array.unsafeNew` *once* and thread
-- it through a single recursion: a working buffer referenced from two places (fill + trim) is
-- duplicated by the optimizer (it treats `unsafeNew` as pure), so each site would allocate its
-- own array and the trim would read an uninitialised one. Counting is a cheap extra pass over a
-- pure predicate, and keeps the single-allocation invariant the other HOFs here rely on too.
filter f xs = go 0 0 (PA.unsafeNew (count 0 0))
  where
  n = PA.length xs
  count i acc = if i == n then acc else count (i + 1) (if f (PA.unsafeIndex xs i) then acc + 1 else acc)
  go i j out =
    if i == n then out
    else
...
partition f xs = { yes: filter f xs, no: filter (not <<< f) xs }
...
scanl f z xs = go 0 z (PA.unsafeNew n)
  where
  n = PA.length xs
  go i acc out =
    if i == n then out
...
scanr f z xs = go (n - 1) z (PA.unsafeNew n)
  where
  n = PA.length xs
  go i acc out =
    if i < 0 then out
...
-- | ```
-- |
-- A stable top-down merge sort over `Purvasm.Array` (the registry `sortByImpl` foreign has no
-- purvasm provider, so without this `sort`/`sortBy` cannot run on purvasm at all). On an `EQ` the
...
sortBy comp xs0 =
  let
    sliceA src lo hi =
      let
        cp i o = if i == hi then o else cp (i + 1) (PA.unsafeSet o (i - lo) (PA.unsafeIndex src i))
      in
        cp lo (PA.unsafeNew (hi - lo))
    merge as bs =
      let
        la = PA.length as
        lb = PA.length bs
        go i j o =
          if i == la then
            let cp k oo = if k == lb then oo else cp (k + 1) (PA.unsafeSet oo (la + k) (PA.unsafeIndex bs k)) in cp j o
          else if j == lb then
            let cp k oo = if k == la then oo else cp (k + 1) (PA.unsafeSet oo (lb + k) (PA.unsafeIndex as k)) in cp i o
          else
            let
              a = PA.unsafeIndex as i
              b = PA.unsafeIndex bs j
            in
              case comp a b of
                GT -> go i (j + 1) (PA.unsafeSet o (i + j) b)
                _ -> go (i + 1) j (PA.unsafeSet o (i + j) a)
      in
        go 0 0 (PA.unsafeNew (la + lb))
    msort a =
      let
        m = PA.length a
      in
...
zipWith
  ...
  -> Array b
  -> Array c
zipWith f xs ys = go 0 (PA.unsafeNew m)
  where
  m = min (PA.length xs) (PA.length ys)
  go i out =
    if i == m then out
    else go (i + 1) (PA.unsafeSet out i (f (PA.unsafeIndex xs i) (PA.unsafeIndex ys i)))
...
any f xs = go 0
  ...
  go i =
    if i == n then false
    else if f (PA.unsafeIndex xs i) then true
    else go (i + 1)
...
all f xs = go 0
  ...
  go i =
    if i == n then true
    else if f (PA.unsafeIndex xs i) then go (i + 1)
    else false
