-- | ulib SHADOW of `enums`' `Data.Enum` (ADR-0038 / ADR-0006), targeting enums 6.0.1.
-- |
-- | The module's only foreigns are `toCharCode`/`fromCharCode`. On this backend a `Char` is already
-- | a code point (ADR-0006), so both are the identity on the underlying `Int` (`Purvasm.Char`); they
-- | are reimplemented in PureScript over `Purvasm.Char` (no native leaf — [[prefer-ulib-over-foreign]]).
...
-- ulib shadow (was a foreign): a `Char` is already a code point on this backend (ADR-0006), so the
-- `Char`/`Int` conversions are the identity on the underlying `Int` (`Purvasm.Char`).
toCharCode :: Char -> Int
toCharCode = PC.toCodePoint

fromCharCode :: Int -> Char
fromCharCode = PC.fromCodePoint
