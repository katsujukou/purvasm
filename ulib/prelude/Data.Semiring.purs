-- | ulib SHADOW of `prelude`'s `Data.Semiring` (ADR-0038), targeting prelude 6.0.2.
-- |
-- | The upstream module's interface (exports, types, instances) is unchanged, so this shadows the
-- | registry module at the user's resolved version. The scalar `Int`/`Number` arithmetic, however,
-- | is reimplemented directly over the `purvasm-base` primitives (`Purvasm.Int`/`Purvasm.Number`)
-- | instead of the registry's `intAdd`/`numMul`/… foreigns, so no opaque foreign remains.
...
-- ulib shadow: `Int`/`Number` arithmetic over the `purvasm-base` primitives (was the registry's
-- `intAdd`/`intMul`/`numAdd`/`numMul` foreigns).
