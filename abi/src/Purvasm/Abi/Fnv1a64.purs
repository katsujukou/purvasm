...
-- |
-- | It lives in `abi` rather than in the compiler because a constructor tag now has **two** consumers:
-- | the LLVM backend mints it when it emits an ADT, and the owned VM mints the same number when a data
-- | value crosses the FFI boundary or a `SwitchCtor` meets one that a leaf returned
-- | ([ADR-0111](../../../../docs/design-decisions/0111-vm-dynamic-native-ffi.md) §3). A second copy of
-- | this arithmetic would agree until it didn't, and the disagreement would be a wrong branch taken
-- | silently. The compiler keeps its `Purvasm.Compiler.Util.Fnv1a64` name as a re-export.
