...
spec = describe "Purvasm.Compiler.Backend.LLVM.Emit" do
  describe "emitFunction" do
    it "emits the tailcc $d entry and the generic wrapper for identInt" do
      let
        ctx = snd $ runCodegen (makeCx { gkeys: Set.empty, xfns: Map.empty, foreignArity: Map.empty, inlineAbi: true, defined: Set.empty, byNeed: true })
    ...
    it "lowers a GER perform as the identical .ll to a one-argument unit call (ADR-0099)" do
      ...
      let
        emitLl body = renderChunks
          ( _.md $ snd $ runCodegen
              (makeCx { gkeys: Set.empty, xfns: Map.empty, foreignArity: Map.empty, inlineAbi: true, defined: Set.empty, byNeed: true })
              ( emitFunction
                  ( Lifted
                      { name: "pv_g_Test_2ef"
                      , params: [ "m" ]
  ...
  describe "emitGcafInit (ADR-0106 slice 2 — the plan-driven fixed-shape init)" do
    let
      gcafLl gkeys e = renderChunks
        ( _.md $ snd $ runCodegen
            (makeCx { gkeys: Set.fromFoldable gkeys, xfns: Map.empty, foreignArity: Map.empty, inlineAbi: true, defined: Set.empty, byNeed: true })
