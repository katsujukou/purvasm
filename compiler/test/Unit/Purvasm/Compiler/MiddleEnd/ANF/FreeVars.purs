...
spec = describe "Purvasm.Compiler.MiddleEnd.ANF.FreeVars" do
  ...
  describe "fvExpr / fvCexpr" do
    ...
    it "treats a lambda's parameters as bound in its body" do
      elems (fvCexpr Set.empty (CLam unit [ "x" ] (Ret (CAtom (AtomVar "x"))))) `shouldEqual` []
    ...
    it "removes let-bound names but keeps the genuinely free ones" do
      ...
      let
        e = Let "a" (CAtom (AtomLit (LInt 0)))
          (Ret (CApp unit (AtomVar "f") [ AtomVar "a", AtomVar "b" ]))
  ...
  describe "cfExpr" do
    it "collects every referenced foreign key and ignores vars/literals" do
      let
        e = Ret (CApp unit (AtomForeign "Data.X.y") [ AtomVar "z", AtomForeign "Data.X.w" ])
    ...
    it "fvExpr walks a 100k-binding spine and still resolves scoping" do
      let
        spine = Array.foldr
          ...
          (Ret (CApp unit (AtomVar "g") [ AtomVar "v0" ]))
