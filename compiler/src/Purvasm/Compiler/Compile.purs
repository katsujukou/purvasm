...
-- | `Pvm.Compile`.
-- |
-- | This is the **`--no-opt`, boot-byte-identical** compile only — the byte-identity test surface
-- | (the E2E `diamond/DiaA` gates). The optimising build lives on the ADR-0087 driver
-- | (`Purvasm.Compiler.build` + `Backend.Bytecode`), which consumes the ADR-0086 seam's optimised
-- | `AnfModule`; the per-binding `--opt` hook this module once carried (`Simplify`, ADR-0028) was
-- | absorbed by the NbE inliner and retired (ADR-0089 §6/§8).
...
-- | Compile a module to its `.pmo` artifact, byte-identical to boot. The `exports` are its
-- | *public* value exports (CoreFn `exports`) intersected with the keys it actually defines, so
-- | private compiler-synthesised bindings stay in the object but off the public surface.
